...
const Slider = (
  ...
  const onMouseMove = (e: MouseEvent) => {
    if (!railEl.current) return;
    const left = e.clientX - shiftHandleX - railEl.current.getBoundingClientRect().left;
    handleSlide(left + 6);
  };
  ...
  const setNewPosition = async (clientX: number) => {
    ...
    await handleSlide(shiftRailX + 6);
